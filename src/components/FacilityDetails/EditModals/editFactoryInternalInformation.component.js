...
const FactoryInternalInformationEditModal = ({
  open = false,
  onClose,
  internalData,
}) => {
    ...
    <Modal open={open} onClose={onClose}>
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          >
            Edit Site Internal Information
          </Typography>
          <Grid container spacing={2} sx={{ mb: 2 }}>
            {Object.keys(initData).map((field, index) => (
              <Grid item xs={12} sm={3} key={internalData?._id}>
                <TextField
                  fullWidth
...
export default FactoryInternalInformationEditModal;
